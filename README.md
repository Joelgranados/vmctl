...
3. There are two ways of executing `vmctl`:
    1. Do a symlink in your path to have it available "natively"

        $ ln -s /path/to/vmctl/vmctl $HOME/bin/vmctl

    2. Always run it with the `env` command:

        $ ./vmctl env --confdir $HOME/vms
        $ vmctl --help

4. Prepare a boot image. The `*-q35-base.conf` configuration will look a base
   ...
   since it is usually shrank to be as small as possible by default.
